...
export default function Navbar({ toggleSidebar }) {
    ...
    const logo2 = aboutData
        ? isAdminLembaga
            ? `/storage/${aboutData.logo2}`
            : aboutData.logo2
        : null;
    ...
    const handleProfile = () => {
        const profileRoute =
            user?.role === "lembaga" ? "/lembaga/profile" : "/staff/profile";
    ...
    return (
        <header className="fixed top-0 left-0 right-0 flex items-center bg-blue-500 text-white p-3 shadow-md z-30 px-8 h-[75px]">
            ...
            <div className="flex-1 flex justify-end items-center space-x-4 relative">
                {/* Hanya tampilkan <img> jika role bukan peserta */}
                {user?.role !== "peserta" && (
                    <div>
                        {user?.role === "lembaga" ? (
                            <img
                        ...
                        <div className="flex items-center space-x-2">
                            <span className="text-base">
                                {user?.role === "lembaga"
                                    ...
                                    : user?.staff?.nama_staff ??
                                      user?.peserta?.nama_peserta ??
                                      "Nama"}
                            </span>
